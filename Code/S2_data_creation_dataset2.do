...
// This varies the amount of missing data without changing the associations with other variables
	matrix missconsmod=(0.25,-4.15,-4.70)
	matrix missconssev=(0.50,-1.70,-2.05)
...
forvalues dat=1/`numdata' {
	...
	forvalues i=0/4 {
		...
		lco[1,1]*lw + lco[1,2]*la + lco[1,3]*obs + ///
		lco[1,4]*oa + lco[1,5]*ob + lco[1,6]*oc + 2.0*v + ew_ijk) if obs==`i'
		
		replace a = runiform()<invlogit(-2.3 + ///
		aco[1,1]*la + aco[1,2]*l + aco[1,3]*ll + aco[1,4]*w + aco[1,5]*lw + aco[1,6]*obs + ///
		aco[1,7]*oa + aco[1,8]*ob + aco[1,9]*oc + ea_ijk) if obs==`i'
		
		replace y = rnormal(5 + ///
		outco[1,1]*la + outco[1,2]*a + outco[1,3]*ll + outco[1,4]*l + outco[1,5]*obs + ///
	...
	save "$cloudstor\PhD\Paper 4 - Missing data simulation\Data\S2\Simulation `dat'.dta", replace
...
global i=$i+1
use "$cloudstor\PhD\Paper 4 - Missing data simulation\Data\S2\Simulation $i.dta", clear
